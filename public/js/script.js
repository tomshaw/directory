/*! demo-app - v1.0.0 (build ) */!function(global, factory) {
...
var Dashboard = function() {
    var dataSets = {}, total = 0, active = 0, initHandlers = function() {
        $("#burger").click(function() {
            ...
            "+" == paginate ? active++ : active--, arrayBounds(), $(".iframe").removeClass("show");
        ...
        active >= total ? active = 0 : 0 >= active && (active = total - 1), console.log(active, "active");
    }, initRandomFrame = function() {
        var frame = randomFrame();
        setTimeout(function() {
            showFrame(frame), $(".iframe").addClass("show");
        }, 500);
    }, normalizePath = function(data) {
        ...
        $(".iframe").addClass("show"), $(".iframe").attr("src", frame);
    }, randomFrame = function() {
        return total = dataSets.length, active = Math.floor(Math.random() * total), arrayBounds(), 
        normalizePath(dataSets[active]);
    };
    return {
        load: function() {
            var that = this;
            $.when($.post("/data")).done(function(response) {
                dataSets = JSON.parse(response), that.init();
            });
        },
        init: function() {
            initHandlers(), initRandomFrame();
        },
